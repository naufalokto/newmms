...
<head>
    ...
    <title>Service History - Mifta Motor Sport</title>
...
<body>
    ...
    <div class="hero-section">
        ...
        <div class="hero-title">Service History</div>
    ...
    @if(session('success'))
        <div style="background: #d4edda; color: #155724; padding: 1rem; margin: 1rem; border-radius: 0.5rem; border: 1px solid #c3e6cb;">
            {{ session('success') }}
        </div>
    @endif

    @if(session('error'))
        <div style="background: #f8d7da; color: #721c24; padding: 1rem; margin: 1rem; border-radius: 0.5rem; border: 1px solid #f5c6cb;">
            {{ session('error') }}
        </div>
    @endif

    <div class="hero-bar">
        <span class="bar-col">Date</span>
        <span class="bar-col">Service Type</span>
    ...
    @if($services->count() > 0)
        @foreach($services as $service)
            <div class="service-list-row" style="display: grid; grid-template-columns: 1.1fr 1.3fr 1.3fr 1fr 2fr 1.2fr; align-items: center; justify-items: center; width: 100%; max-width: 1100px; margin: 0 auto 1.9rem auto; padding: 0 40px; min-height: 55px; border-bottom: 1px solid #eee;">
                ...
                <span>
                    @if($service->status === 'pend')
                        <form method="POST" action="{{ route('service.cancel', $service->id_service) }}" style="display: inline;">
                            @csrf
                            <button type="submit" class="cancel-btn-red" onclick="return confirm('Are you sure you want to cancel this appointment?')">
                                Cancel Appointment
                            </button>
                        </form>
                    @else
                        <span style="color: #999; font-size: 0.9rem;">No action available</span>
                    @endif
                </span>
            </div>
        @endforeach
    @else
        <div style="text-align: center; padding: 2rem; color: #666; font-family: 'Poppins', sans-serif;">
            <p>No service bookings found.</p>
            <a href="/" style="color: #FE8400; text-decoration: none;">Book a service now</a>
        </div>
    @endif
